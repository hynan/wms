      ...
      单据号：{{info.basicInfo.damageBillCode}}
        ...
        <el-row style="padding: 5px;margin-top: 10px">
          <el-steps :active="progress" align-center finish-status="success">
            <el-step v-for="item in damageBillStatus" :title="item" :key="item"></el-step>
          </el-steps>
        </el-row>
        ...
        doAjax(){
            this.$http.post('/ys-web-wms/damage/getInfo',{id:this.$route.params.id}).then(
                (res)=>{
                    ...
                    this.damageBillStatus=(res.data.data&&res.data.data.progressBar)?res.data.data.progressBar.progressNameArray:[]
                }
            )
        }
    },
