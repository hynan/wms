...
<script>
    ...
    export default {
        ...
        methods:{
            print(){
                if(!this.printData||this.printData.length<1){
                    this.$http.post("/ys-web-wms/store/getAllocationByWs",{id:this.$route.params.id})
                        .then((response) => {
          ...
          delStore(){
              this.$confirm('确认删除该项?', '提示', {
                  ...
                  this.$http.post("/ys-web-wms/store/delStore", {id:this.$route.params.id})
                      .then((response) => {
        ...
        computed:{
            "edit":function(){
                return "/dataManagement/store/detail/store/"+this.$route.params.id+"/edit";
            },
        },
        watch:{
            '$route.params.id'(){
