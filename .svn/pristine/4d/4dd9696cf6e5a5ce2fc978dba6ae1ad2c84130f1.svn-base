<template>
    <div class="dataManage-navbtn-wrapper">
        ...
        <el-button class="el-button el-button--mini" @click="print" type="primary">打印货位条码</el-button>
...
<script>
    ...
    export default {
        ...
        methods:{
            print(){
                if(!this.printData||this.printData.length<1){
                    this.$http.post("/ys-web-wms/store/getAllocationByShelf",{id:this.$route.params.id})
                        .then((response) => {
            ...
            delStore(){
                this.$confirm('确认删除该项?', '提示', {
                    ...
                    this.$http.post("/ys-web-wms/store/delContainer", {id:this.$route.params.id})
                        .then((response) => {
        ...
        computed:{
            "edit":function(){
                return "/dataManagement/store/detail/container/"+this.$route.params.id+"/edit";
            },
        },
        watch:{
            '$route.params.id'(){
