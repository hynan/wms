      ...
      <div class="right-content-body">
        <el-row style="padding: 5px;margin-top: 10px">
          <el-steps :active="progress" align-center finish-status="success">
            <el-step :title="item" v-for="item,index in transferBillStatus" :key="index"></el-step>
          </el-steps>
        </el-row>
          ...
          doAjax(){
              this.$http.post('/ys-web-wms/transfer/getInfo',{id:this.$route.params.id}).then(
                  (res)=>{
                      ...
                      this.transferBillStatus = (res.data.data&&res.data.data.progressBar)?res.data.data.progressBar.progressNameArray:[]
                  }
              )
          }
      },
